    ...
    public static class L4Logger
    {
        ...
        public static ILog Logger
        {
            get
            {
                if (logger == null)
                {
                    var repository = LogManager.CreateRepository("Daricheh.XFS");

                    logger = log4net.LogManager.GetLogger("Daricheh.XFS", "Daricheh.XFS");

                    RollingFileAppender fileAppender = new RollingFileAppender
                    {
                        Name = "LogFileAppenderDevice",
                        Layout = new PatternLayout("%d [%t] %logger {%property{method}}  [%property{NDC}]  ->  %m%n"),
                        File = @"D:\Daricheh\logs\DeviceManager\XFS\",
